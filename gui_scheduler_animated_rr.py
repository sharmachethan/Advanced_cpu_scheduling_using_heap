...
from scheduler_code_rr import Process, ProcessScheduler, generate_random_processes
...
class SchedulerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Heap-Based CPU Scheduler")
        ...
        self.paused = False
        self.speed = 1.0
        self.quantum_value = 2  # default time quantum

        # =================== TITLE =====================
        title = tk.Label(root, text="Heap-Based CPU Scheduler - Animated Gantt Chart with Idle Time and RR Support",
        ...
        labels = ["PID", "Arrival", "Burst", "Priority", "Type"]
        for i, text in enumerate(labels):
            tk.Label(frame_top, text=text, **style_lbl).grid(row=0, column=i, padx=5)
        ...
        entries = [self.pid, self.arrival, self.burst, self.priority, self.ptype]
        ...
        tk.Button(frame_top, text="Clear Simulation", command=self.clear_all,
                  bg="#cc0000", fg="white", font=("Segoe UI", 9, "bold")).grid(row=1, column=7, padx=8)
        ...
        self.algorithm = ttk.Combobox(frame_options, values=["priority", "sjf", "fcfs", "rr"], width=15, justify="center")
        self.algorithm.current(0)
        self.algorithm.grid(row=0, column=3, padx=5)
        self.algorithm.bind("<<ComboboxSelected>>", self.toggle_quantum_field)

        self.quantum_label = tk.Label(frame_options, text="Time Quantum:", bg="#eaf0f6", font=("Segoe UI", 10, "bold"))
        self.quantum_entry = tk.Entry(frame_options, width=5, justify="center")
        ...
        style.configure("mystyle.Treeview",
                        font=("Segoe UI", 10), rowheight=26, background="white", fieldbackground="white")
        style.configure("mystyle.Treeview.Heading",
                        font=("Segoe UI", 11, "bold"), background="#00509E", foreground="white")

        border_frame = tk.Frame(table_frame, bg="#004080", bd=2, relief="ridge")
        border_frame.pack(padx=40, pady=5, fill="x")

        self.tree = ttk.Treeview(border_frame, style="mystyle.Treeview",
    ...
    def toggle_quantum_field(self, event=None):
        algo = self.algorithm.get()
        if algo == "rr":
            self.quantum_label.grid(row=0, column=9, padx=5)
            self.quantum_entry.grid(row=0, column=10, padx=5)
        else:
            self.quantum_label.grid_forget()
            self.quantum_entry.grid_forget()
    ...
    def add_process(self):
        try:
            ...
            if not pid:
                ...
                return
            burst = self.burst.get().strip()
            if not burst:
                messagebox.showerror("Error", "Burst time is required.")
                return
            ...
            for entry in [self.pid, self.arrival, self.burst, self.priority]:
                entry.delete(0, tk.END)
        except ValueError:
            messagebox.showerror("Error", "Enter valid numeric values for burst/arrival/priority.")
    ...
    def clear_all(self):
        """Clear process list, table, chart, and metrics."""
        ...
        messagebox.showinfo("Cleared", "Simulation cleared. You can add new processes now.")
    ...
    def run_scheduler_animated(self):
        ...
        self.running = True
        try:
            quantum = int(self.quantum_entry.get()) if self.algorithm.get() == "rr" else 2
        except ValueError:
            quantum = 2

        scheduler = ProcessScheduler(self.processes, mode=self.mode.get(),
                                     algorithm=self.algorithm.get(), quantum=quantum)
        scheduler.schedule()

        for i in self.tree.get_children():
            self.tree.delete(i)
        for p in scheduler.completed:
            self.tree.insert("", "end", values=(p.pid, p.process_type, p.arrival, p.burst, p.priority,
                                                p.start, p.finish, p.waiting, p.turnaround, p.response_time))

        self.animate_gantt_chart(scheduler)
        n = len(scheduler.completed)
        total_wait = sum(p.waiting for p in scheduler.completed)
        ...
        self.metrics.config(text=f"CPU Utilization: {scheduler.cpu_utilization:.2f}%   |   "
                                 f"Total Waiting: {total_wait:.2f}   |   "
                                 f"Avg Waiting: {avg_wait:.2f}   |   "
                                 f"Avg Turnaround: {avg_turn:.2f}   |   "
    ...
    def animate_gantt_chart(self, scheduler):
        self.ax.clear()
        self.ax.set_title("Animated Gantt Chart", fontsize=11)
        ...
        for start, finish, pid in timeline:
            if start > current_time:
                for t in range(current_time, start):
                    ...
                    self.ax.set_xticks(range(0, int(finish) + 2))
